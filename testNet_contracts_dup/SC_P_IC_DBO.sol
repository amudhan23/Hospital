...
contract SC_P_IC_DBO
{
    ...
    function returnClaimIds(uint _pId) view public returns(uint[] memory)
    {
    ...
    function claimMoney(address System_Users_Info_address,address SC_P_HA_1_address,uint _policyID, uint _claimedAmount,uint _estimatedBillId,bytes memory _publicKey, bytes32 _commitmentOfSecretKey,address _dboAddr) public
    {
        ...
        SC_P_HA_1.FinalCheckUpCost memory finalCheckUpCost_Object=SC_P_HA_1_instance.getFinalBillStruct(_finalBillID);



        require(_pID==finalCheckUpCost_Object.pID);
        ...
        cd=ClaimDetails(cidGenerator,_estimatedBillId,now,_claimedAmount,0,_publicKey,_commitmentOfSecretKey,'',0,0,0,0,false);
