...
contract SC_P_DBO
{
		...
		address payable public databaseOwnerAddr;
    ...
    ApplicationForStoring[] public applicationForStoring;
		...
		function initializeDatabaseOwner() public
		{
			databaseOwnerAddr=msg.sender;
		}

		function keepData_Patient(address System_Users_Info_address,address P_HA_address,uint _estimatedBillID,bytes32 _merkleRootEncFile, bytes32 _merkleRootFile) payable public returns(uint)
    {
        uint _pId;
				uint _multiSigID;
				uint _finalBillID;
        ...
        require(finalCheckUpCost_Object.timeStampOfComplain==0);
				...
				ApplicationForStoring memory a=ApplicationForStoring(applicationIDGenerator,_multiSigID,"",multiSigOnMedicalData_Object.hashOfData,_merkleRootEncFile,now,0,0,0,0,0);
		...
		function approval_DO(uint _appID) public
		{
			...
			require(applicationForStoring[_appID-1].timestamp_complainOrApproval==0);
			require(now-applicationForStoring[_appID-1].timestamp_keyReveal<=timeLimitForGivingComplainOrPuttingInTheDatatbase_P_DO);
			applicationToApproval[_appID-1]=true;
		...
		function verify(bytes32[][] memory complaint,bytes[] memory encodedVectors_ThisGate,uint _outputIndex,uint _depth,bytes32 _hashOfEncryptedData,bytes32 _key) public returns(bool){
		...
		function  Mverify(bytes32[] memory complaint,uint _index,uint _depth,bytes32 _hashOfEncryptedData) internal view returns(bool)
		{
			bytes32 _value;
			_value=complaint[0];
			bytes32 Hashed;
		...
		function Operation(bytes[] memory inputVectorsToTheGate) internal pure returns(bytes32)
		{
				bytes32 Hashed;
				bytes memory toBeHashed;
