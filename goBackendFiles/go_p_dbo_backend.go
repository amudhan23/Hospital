...
func main(){
			...
			logFilePath:="../log_backend/log_p_dbo_backend.txt"
			...
			con_address_P_DBO,_,_,err=import_contract_p_dbo.DeploySCPDBO(govtAuthority,client)
			if err!=nil{
			    log.Fatal(err)
			}
			client.Commit()
	...
	func Patient(patient *bind.TransactOpts,client *backends.SimulatedBackend,channel_HospitalToPatient_EstimatedBillId chan int,channel_HospitalToPatient_EncryptedFile_Object chan []byte,channel_PatientToHospital_PatientId chan int,channel_HospitalToPatient_HospitalId chan int,channel_multiSignId chan int,wg *sync.WaitGroup){
		...
		instance_P_HA,err:=import_contract_p_dbo.NewSCPHA1(con_address_P_HA_1,client)
		if err!=nil{
		    log.Fatal(err)
		}

		instance_P_HA_Leave,err:=import_contract_p_dbo.NewSCPHA2(con_address_P_HA_2,client)
		if err!=nil{
		    log.Fatal(err)
		}
