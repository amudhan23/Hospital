...
import(
	  ...
	  "github.com/ethereum/go-ethereum/common"
		...
		"math/big"
		 mathlib "math"
		 // import_contract_p_h "./contract_registration"
...
func main(){
			...
			logFilePath:="../log_testNet/log_p_h_testNet.txt"
			...
			// contractDeployer:=bind.NewKeyedTransactor(contractDeployer_PrivateKey)


			con_address_registration = common.HexToAddress("Address")
			con_address_P_HA_1 = common.HexToAddress("Address")
			con_address_P_HA_2 = common.HexToAddress("Address")
				// 0xA11Cfe5a1bdF0ff1c1a314C2a8C48b6e6DD55B12
			// 0x8a08F983F9B4613dFfC46FB6C286B96A9D43eebD
			// 0xd88f48380C40837d0A550Eb4b1df638d4e8c0849
			...
			go Patient(patient,client,channel_HospitalToPatient_EstimatedBillId,channel_HospitalToPatient_EncryptedFile_Object,channel_PatientToHospital_PatientId,channel_HospitalToPatient_HospitalId,channel_multiSignId,ctx,&wg)

			go Hospital(hospital,client,channel_HospitalToPatient_EncryptedFile_Object,channel_HospitalToPatient_EstimatedBillId,channel_PatientToHospital_PatientId,channel_HospitalToPatient_HospitalId,channel_HospitalToPatient_sendHashDateMRenc,channel_multiSignId,hospital_PrivateKey,ctx,&wg)
	...
	func Patient(patient *bind.TransactOpts,client *ethclient.Client,channel_HospitalToPatient_EstimatedBillId chan int,channel_HospitalToPatient_EncryptedFile_Object chan []byte,channel_PatientToHospital_PatientId chan int,channel_HospitalToPatient_HospitalId chan int,channel_multiSignId chan int,ctx context.Context,wg *sync.WaitGroup){
		...
		instance_PHA2,err:=import_contract_p_h.NewSCPHA2(con_address_P_HA_2,client)
		if err!=nil{
				log.Fatal(err)
		}
		...
		patient_LockEstimatedAmount_start := time.Now()
								...
								tx,err=instance_PHA1.LockEstimatedAmount(&bind.TransactOpts{
										From:patient.From,
		...
		log.Println("Tx Patient_LockingEstimatedAmount : ",tx.Gas(),"\n"," txHash : ",tx.Hash().Hex(),"\n")
			...
			patient_WaitingForHospitalToStartTreatment_start:=time.Now()
							...
							for i:=0;i<timeLimitForStartingOperation;i++ {
								...
								estimatedCheckUpCost_Object,err:=instance_PHA1.EstimatedCheckUpCost(nil,big.NewInt(int64(estimatedBillId-1)))
								if err!=nil{
										log.Fatal(err)
								}
			...
			if !patient_hospitalStartingTreatment{
					...
					patient_UnlockingEstimatedAmount_HospitalNotStartedTreatment_start:=time.Now()
									...
									tx,err=instance_PHA2.PatientLeaveLockEstimatedAmount(&bind.TransactOpts{
												From:patient.From,
												Signer:patient.Signer,
				...
				patient_WaitingForHospitalToSendMultiSignIdAndEncryptedFiles_start:=time.Now()
							...
							patient_MultiSigOnMedicalDataObject,err:=instance_PHA1.MultiSigOnMedicalData(nil,big.NewInt(int64(multiSigID-1)))
							if err!=nil{
									log.Fatal(err)
							}
			...
			if bool_verification_merkleRoots_encFile && verification_hashDatePatient_signature{
						...
						patient_VerifyingSignatureAndMerkleRoot_start:=time.Now()
									...
									tx,err=instance_PHA1.VerifyAndGiveConsent(&bind.TransactOpts{
												From:patient.From,
												Signer:patient.Signer,
			...
			patient_WaitingForHospitalToCommitFinalBill_start:=time.Now()
								...
								for i:=0;i<timeLimitForGeneratingFillBill;i++ {
										...
										finalBillID,err=instance_PHA1.EstimatedBillIDToFinalBillID(nil,big.NewInt(int64(estimatedBillId)))
										if err!=nil{
												log.Fatal(err)
										}
			...
			if !patient_hospitalGeneratingFinalBill{
					  ...
					  patient_LeavingSinceHospitalNotCommitedFinalBill_start:=time.Now()
										...
										MultiSigOnMedicalData_Object,err:=instance_PHA1.MultiSigOnMedicalData(nil,big.NewInt(int64(multiSigID-1)))
										if err != nil {
											log.Fatal(err)
										}
										...
										tx,err=instance_PHA2.PatientLeaveConsentFinalBill(&bind.TransactOpts{
													From:patient.From,
													Signer:patient.Signer,
			...
			patient_GiveConsentFinalBill_start:=time.Now()
							...
							tx,err=instance_PHA1.ConsentFinalBillPatient(&bind.TransactOpts{
										From:patient.From,
										Signer:patient.Signer,
			...
			patient_WaitingForHospitalToRevealKey_start:=time.Now()
							...
							for i:=0;i<timeLimitForKeyRevealing;i++ {
									...
									keyAndExchange_object,err:=instance_PHA1.MultiSignIdToKeyAndExchange(nil,big.NewInt(int64(multiSigID)))
									if err!=nil{
											log.Fatal(err)
									}
			...
			if !patient_hospitalKeyReveal{
					...
					patient_LeavingSinceHospitalNotRevealedKey_start:=time.Now()
									...
									finalBill_Object,err:=instance_PHA1.FinalCheckUpCost(nil,big.NewInt(finalBillID.Int64()-1))
									if err != nil {
										log.Fatal(err)
									}
			...
			if complain==nil{
					...
					patient_FinalConsent_start:=time.Now()
								...
								tx,err:=instance_PHA1.PatientFinalConsent(&bind.TransactOpts{
										From:patient.From,
										Signer:patient.Signer,
				...
				if complain !=nil{
								...
								patient_Complain_start:=time.Now()
											...
											tx,err:=instance_PHA1.PatientComplain(&bind.TransactOpts{
													From:patient.From,
													Signer:patient.Signer,
	...
	func Hospital(hospital *bind.TransactOpts,client *ethclient.Client,channel_HospitalToPatient_EncryptedFile_Object chan []byte,channel_HospitalToPatient_EstimatedBillId chan int,channel_PatientToHospital_PatientId chan int,channel_HospitalToPatient_HospitalId chan int,channel_HospitalToPatient_sendHashDateMRenc chan [hashFunctionOutputBitSize]byte,channel_multiSignId chan int,hospitalprivateKey *ecdsa.PrivateKey,ctx context.Context,wg *sync.WaitGroup){
				...
				hospital_GenerateEstimateBill_start := time.Now()
						  ...
						  tx,err=instance_PHA1.GenerateEstimatedCostBill(&bind.TransactOpts{
									From:hospital.From,
				...
				hospital_SendingEstimateBill_start := time.Now()
							...
							estimatedBillId,err:=instance_PHA1.PatientIdHospitalIdEstimateBillId(nil,big.NewInt(int64(patientId)),hospitalId)
							if err!=nil{
									log.Fatal(err)
							}
				...
				hospital_WaitingForPatientToLockEstimateAmount_start:=time.Now()
								...
								for i:=0;i<timeLimitForPatientLocking;i++{
										...
										estimatedCheckUpCost_Object,err:=instance_PHA1.EstimatedCheckUpCost(nil,big.NewInt(estimatedBillId_int-1))
										if err!=nil{
												log.Fatal(err)
										}
				...
				if !hospital_lockingByPatient {
						...
						hospital_LeavingSincePatientNotLockedEstimateAmount_start:=time.Now()
											...
											fmt.Println("\n Now : ",Now," TimestampOfLockingByHospital+timeLimitForPatientLocking : ",TimestampOfLockingByHospital.Int64()+timeLimitForPatientLocking)




											tx,err=instance_PHA2.HospitalLeaveGenerateEstimatedCostBill(&bind.TransactOpts{
														From:hospital.From,
														Signer:hospital.Signer,
				...
				hospital_StartTreatment_start:=time.Now()
							...
							tx,err=instance_PHA1.StartTreatment(&bind.TransactOpts{
										From:hospital.From,
										Signer:hospital.Signer,
		    ...
		    hospital_KeepingSignedHashToBlockchain_start:=time.Now()
								...
								tx,err=instance_PHA1.KeepSignedHashToBlockchain(&bind.TransactOpts{
										From:hospital.From,
										Signer:hospital.Signer,
				...
				hospital_SendingMultiSignIdAndEncodedOuptuts_start:=time.Now()
									...
									multiSignId,err:=instance_PHA1.EstimatedCostBillIDToMultiSigOnMedicalDataID(nil,estimatedBillId)
									if err!=nil{
											log.Fatal(err)
									}
				...
				hospital_WaitingForPatientToVerifySignatureAndMerkleRoot_start:=time.Now()
								...
								for i:=0;i<timeLimitForVerifyingSignature;i++ {
									...
									multiSignObject,err:=instance_PHA1.MultiSigOnMedicalData(nil,big.NewInt(multiSignId_int-1))
									if err != nil {
										log.Fatal(err)
									}
				...
				if !hospital_VerifyingSignature {
							...
							hospital_UnlockingEstimatedAmountSincePatientNotVerifySignature_start:=time.Now()
											...
											MultiSigOnMedicalData_Object,err:=instance_PHA1.MultiSigOnMedicalData(nil,big.NewInt(multiSignId_int-1))
											if err != nil {
												log.Fatal(err)
											}
											...
											for  {
																		 ...
																		 if Now.Int64()>TimeStampOfSignByHospital.Int64()+timeLimitForVerifyingSignature{
																				break
																		 }
											...
											tx,err=instance_PHA2.HospitalLeaveKeepHashToBlockchain(&bind.TransactOpts{
														From:hospital.From,
														Signer:hospital.Signer,
											},con_address_registration,con_address_P_HA_1,big.NewInt(int64(patientId)),estimatedBillId)
					...
					hospital_DischargeAndGenerateFinalBill_start:=time.Now()
									...
									tx,err=instance_PHA1.DischargeAndGenerateFinalCostBill(&bind.TransactOpts{
											From:hospital.From,
											Signer:hospital.Signer,
											// Value:big.NewInt(50),
					...
					finalBillID,err:=instance_PHA1.EstimatedBillIDToFinalBillID(nil,estimatedBillId)
					if err!=nil{
							log.Fatal(err)
					}
					...
					hospital_WaitingForPatientToGiveConsentFinalBill_start:=time.Now()
									...
									for i:=0;i<timeLimitForSigningFinalBill;i++ {
											...
											finalBill_Object,err:=instance_PHA1.FinalCheckUpCost(nil,big.NewInt(finalBillID_int-1))
											if err != nil {
												log.Fatal(err)
											}
					...
					if !hospital_ConsentFinalBill {
						...
						hospital_UnlockingFinalAmountSincePatientNotConsent_start:=time.Now()
									...
									tx,err=instance_PHA2.HospitalLeaveDischargeAndGenerateFinalCostBill(&bind.TransactOpts{
												From:hospital.From,
												Signer:hospital.Signer,
				...
				hospital_KeyReveal_start:=time.Now()
							...
							tx,err=instance_PHA1.KeyReveal(&bind.TransactOpts{
										From:hospital.From,
										Signer:hospital.Signer,
