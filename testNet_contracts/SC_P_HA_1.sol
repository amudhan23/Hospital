...
contract SC_P_HA_1
{
    ...
    modifier only_SC_P_HA_2(address _party)
    {
		...
		function getExcessOrDefecit(uint id) public view returns(ExcessOrDeficit memory)
		{
			return(excessOrDeficit[id-1]);
		}
    ...
    function setFinalCheckUpCost(FinalCheckUpCost memory finalCheckUpCost_Object,uint id) only_SC_P_HA_2(msg.sender) public
    {
		...
		function setExcessOrDeficit(ExcessOrDeficit memory excessOrDeficit_object,uint id) only_SC_P_HA_2(msg.sender) public
		{
				excessOrDeficit[id-1]=excessOrDeficit_object;
		}

    function incrementAndSetExcessOrDeficit(ExcessOrDeficit memory excessOrDeficit_object,uint _finalId) only_SC_P_HA_2(msg.sender) public
		{
				excessOrDeficitIDGenerator+=1;
        excessOrDeficit_object.edID=excessOrDeficitIDGenerator;
        finalCheckUpCost[_finalId-1].excessOrDeficitID=excessOrDeficitIDGenerator;
        excessOrDeficit.push(excessOrDeficit_object);
		}
